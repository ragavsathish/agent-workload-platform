...
import { parseMermaidToExcalidraw } from "../src/index.js";
...
id1[Database]`);
  ...
  it("converts C4 deployment diagrams into editable flowchart elements", async () => {
    const definition = `C4Deployment
  title Editable deployment
  ...
  Rel(builder, wassette, "Loads component", "MCP")`;

    const graph = await parseMermaid(definition);
    expect(graph.type).toBe("flowchart");

    const result = graphToExcalidraw(graph);
    expect(result.elements.some((element) => element.type === "image")).toBe(
      false
    );
    expect(
      result.elements.filter((element) => element.type === "rectangle")
    ).toHaveLength(5);
    expect(
      result.elements.filter((element) => element.type === "arrow")
    ).toHaveLength(2);
    expect(JSON.stringify(result.elements)).toContain("Coordinator Pi");
    expect(JSON.stringify(result.elements)).toContain("Submits build");

    const withTitle = await parseMermaidToExcalidraw(definition);
    expect(withTitle.elements[0]).toMatchObject({
      id: "c4_title",
      type: "text",
      text: "Editable deployment",
    });
  });
